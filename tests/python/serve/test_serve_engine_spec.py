...
def test_engine_basic():
    ...
    ssm = ModelInfo("Llama-2-7b-chat-hf-q4f16_1")
    model = ModelInfo("Llama-2-7b-chat-hf-q0f16")
    ...
    # Create engine
    engine = Engine([model, ssm], kv_cache_config, fcallback)
...
def test_engine_continuous_batching_1():
    ...
    # Create engine
    timer = CallbackTimer()
    engine = Engine([model, ssm], kv_cache_config, timer.callback_getter())
...
def test_engine_generate():
    ...
    kv_cache_config = KVCacheConfig(page_size=16)
    # Create engine
    engine = Engine([model, ssm], kv_cache_config)
...
def test_engine_efficiency():
    ...
    # Initialize model loading info and KV cache config
    model = ModelInfo("Llama-2-13b-chat-hf-q4f16_1")
    kv_cache_config = KVCacheConfig(page_size=16)
...
def test_engine_spec_efficiency():
    ...
    # ssm = ModelInfo("TinyLlama-1.1B-Chat-v1.0-q0f16")
    model = ModelInfo("Llama-2-13b-chat-hf-q4f16_1")
    kv_cache_config = KVCacheConfig(page_size=16)
    ...
    # Create engine
    spec_engine = Engine([model, ssm], kv_cache_config, fcallback, spec_decode_draft_length=6)
